...
def runDNN(args):
	...
	if(args.network == "default"):
	...
	elif(args.network == "clusterShapeOnly"):
		...
		nepochs = 1500
		early = False 
	elif(args.network == "IsoOnly"):
		#isolation only
		benchmark = ["sample","event","object","subcl","ecalPFClusterIsoOvPt","hcalPFClusterIsoOvPt","trkSumPtHollowConeDR03OvPt","label"]
		network_name += "_"+args.network
	elif(args.network == "subclusterObs"):
		...
		network_name += "_"+args.network
	elif(args.network == "addTimeEtaCov"):
		#with time-eta cov
		...
		network_name += "_"+args.network
	elif(args.network == "addTimeEtaCovEtaPhiCov"):
		#with time-eta cov
		benchmark = ["sample","event","object","subcl","R9","Sietaieta","Smajor","Sminor","ecalPFClusterIsoOvPt","hcalPFClusterIsoOvPt","trkSumPtHollowConeDR03OvPt","timeeta_cov","etaphi_cov","label"]
		network_name += "_"+args.network
	else:
		print("Invalid network selected",args.network)

	print("features used",benchmark)	
	data = data[benchmark]
